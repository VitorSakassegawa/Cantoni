...
export default function DocumentAcceptanceForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Falha ao registrar aceite.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-[1.5rem] border border-emerald-100 bg-emerald-50 p-6 print:hidden">
      ...
      <input value={acceptanceName} onChange={(e) => setAcceptanceName(e.target.value)} className="h-12 w-full rounded-2xl border border-emerald-200 bg-white px-4 text-sm font-medium text-slate-700 outline-none" placeholder="Seu nome completo" required />
