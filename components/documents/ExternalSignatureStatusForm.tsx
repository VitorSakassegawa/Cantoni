...
export default function ExternalSignatureStatusForm({
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Nao foi possivel atualizar a assinatura externa.')
      }
      ...
      toast.error(
        error instanceof Error ? error.message : 'Falha ao atualizar assinatura externa.'
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-4 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-5 print:hidden">
      ...
      <select value={status} onChange={(e) => setStatus(e.target.value as ExternalSignatureStatus)} className="h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-bold text-slate-700 outline-none">
        {OPTIONS.map((option) => (
