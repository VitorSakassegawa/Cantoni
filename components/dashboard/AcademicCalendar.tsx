...
export default function AcademicCalendar({ isProfessor = false }: Props) {
  ...
  if (loading) return <div className="h-96 flex items-center justify-center text-slate-400 font-bold uppercase tracking-widest text-xs">Carregando calendário...</div>
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-2 bg-white p-1.5 rounded-full border border-slate-100 shadow-sm">
           <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-orange-50 text-orange-700 text-[11px] font-black uppercase tracking-wider">
              <Umbrella className="w-3.5 h-3.5" /> Recesso / Férias
           </div>
           <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-rose-50 text-rose-700 text-[11px] font-black uppercase tracking-wider">
            ...
            <div key={mIdx} className="glass-card border-none p-5 relative overflow-hidden group">
              ...
              <div className="grid grid-cols-7 gap-1 text-center">
                {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((d, i) => (
                  <div key={i} className="text-[11px] font-black text-slate-300 py-1 uppercase">{d}</div>
                ))}
                ...
                {days.map((day, dIdx) => {
                  ...
                  return (
                    <div 
                      key={dIdx} 
                      className={`
                        aspect-square flex items-center justify-center text-xs font-bold rounded-lg transition-all relative cursor-default
