...
export default function ManageRecessoModal({ onSuccess, existingRecessos }: Props) {
  ...
  const [tipo, setTipo] = useState<'feriado' | 'recesso'>('recesso')
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="space-y-4 md:col-span-2">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">Título do Evento</Label>
                  <Input 
                    placeholder="Ex: Férias Julho, Carnaval..." 
             ...
             <div className="space-y-2 md:col-span-2">
               ...
               <Select value={tipo} onChange={e => setTipo(e.target.value as any)} className="h-14 rounded-2xl bg-slate-50 border-slate-100 font-bold">
                 <option value="recesso">Recesso / Férias do Professor</option>
               ...
               {tipo === 'recesso' && (
                 <p className="text-[9px] text-orange-600 font-bold mt-2 uppercase tracking-tight flex items-center gap-1">
                   <Umbrella className="w-3 h-3" /> Aulas neste período serão marcadas para remarcação.
                 </p>
               )}
             </div>
             ...
             <Button 
               disabled={loading}
               className="md:col-span-2 h-14 rounded-2xl lms-gradient text-white font-black text-xs uppercase tracking-[0.2em] shadow-lg shadow-blue-500/20"
             >
